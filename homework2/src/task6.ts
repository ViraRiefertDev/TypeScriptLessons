...
if (confirm('Почати тестування?')) {
  ...
  function getDistanceBtwCities([lat1,lon1]:Coord, [lat2,lon2]:Coord):number{
    const dx = lat1 - lat2
    const dy = lon1 - lon2
    return Math.hypot(dx,dy)
  }

  function getClosestPlace(target:Coord, markers: Marker[]):Marker{
    if(markers.length === 0) throw new Error('Список міст порожній')
    
    let best:Marker = markers[0]
  ...
  function getMarkersList(markersNumber:number):Marker[]{
    ...
    for(let i=0; i<markersNumber; i++){
      ...
      if(!raw) throw new Error('Скасовано користувачем')
  ...
  try{
    ...
    if (!rawTarget) throw new Error('Скасовано користувачем');
    const [tLatStr, tLonStr] = rawTarget.trim().split(/\s+/);
    const target: Coord = [Number(tLatStr), Number(tLonStr)];
