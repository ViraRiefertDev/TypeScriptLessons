...
if (confirm('Почати тестування?')) {
    ...
    let Season;
    (function (Season) {
        Season[Season["Summer"] = 6] = "Summer";
        Season[Season["Spring"] = 3] = "Spring";
        Season[Season["Fall"] = 9] = "Fall";
        Season[Season["Winter"] = 12] = "Winter";
    })(Season || (Season = {}));
    /**
    ...
    function parseMonth(input) {
        if (input === null)
            fail('Скасовано користувачем');
        const number = Number(input.trim());
        if (Number.isNaN(number))
            fail('Ви ввели не число');
        if (!Number.isInteger(number))
            fail('Потрібне ціле число');
        if (number < 1 || number > 12)
            fail('Місяць має бути від 1 до 12');
        return number;
    }
    /**
     ...
     * @returns сезон
     */
    function getSeasonOfMonth(monthNum) {
        switch (monthNum) {
